...
// Подключаем заголовки, чтобы иметь возможность делать dynamic_cast в тестах
#include "Chest.h"
#include "Reward.h"
#include "Trap.h"
#include "Entity.h"
...
int main() {
    ...
    // Проверяем, как сработал рандом внутри монеты
    ...
    // Проверяем, как сработал рандом внутри оружия (урон и стихия)
    ...
    // Ловушки должны переключить активность при тике
    ...
    // Проверяем функцию полной очистки менеджера (вызовет деструкторы для всех объектов на поле)
    OBJ_MGR.clear_field();
