...
//                                        Base Trap
//-------------------------------------------------
Trap::Trap() : Obj(), is_active(true) { // По умолчанию ловушки начинают активными
...
void Trap::tick_eff() {
    // Сначала вызываем базовый тик Obj, чтобы обработать яд/огонь на ловушке, если они есть
    Obj::tick_eff();

    // Переключаем модификатор активности ловушки во время тика менеджера объектов
    is_active = !is_active;
...
Spikes::Spikes() : Trap() {
    ...
    // Теперь прочность шипов напрямую является их мгновенным уроном
...
int Spikes::get_spikes_damage() const {
    return soundnes; // Возвращаем soundnes как показатель урона
}
